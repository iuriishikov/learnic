...
class StorageQuotaEnforcedSpec(
    ...
    push_body: ClassVar[str] = (
        "Льготный период истёк — часть свежих файлов удалена для "
        "соответствия лимиту тарифа."
    )
    email_subject: ClassVar[str] = "Файлы удалены по лимиту тарифа"
    email_body: ClassVar[str] = (
        "Льготный период по превышению хранилища истёк. Самые свежие "
        "загрузки были удалены, чтобы привести занимаемое место "
        "к лимиту вашего текущего тарифа. Восстановить файлы можно "
        "обратившись в поддержку, если они ещё не очищены "
        "финальным сборщиком мусора."
    )
