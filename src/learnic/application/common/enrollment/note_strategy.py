...
@final
class NoteEnrollmentStrategy(EnrollmentStrategy):
    """Concrete strategy for ``EnrollmentKind.NOTE``.

    Pre-conditions:

    * Product exists and supports
      ``ProductCapability.HAS_NOTE_ENROLLMENT`` (i.e. is a
      ``NOTE``-kind product).
    * The note has at least one release — otherwise the
      enrollment can't be pinned to a release version.

    Constructs the :class:`Enrollment` via
    ``Enrollment.create_note`` (which also builds the
    ``NoteEnrollmentDetails`` body) and stages persistence
    ...
    async def find_existing(
        ...
        assert isinstance(target, NoteEnrollmentTarget)  # noqa: S101
    ...
    async def enroll(
        ...
        if latest_release is None:
            raise CannotEnrollInUnreleasedNoteError(target.product_id)
