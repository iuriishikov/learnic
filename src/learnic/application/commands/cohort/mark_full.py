...
class MarkCohortFullCommand:
    actor_id: UserID
    cohort_id: CohortID


@final
class MarkCohortFullCommandHandler:
    """Sets a cohort's enrollment status to ``FULL``.

    Usually triggered automatically when ``max_participants`` is
    reached during enrollment, but exposed as a manual override
    for cases where the cap is enforced out-of-band (waiting
    list, manual approval, etc.).
    """
    ...
    async def run(self, data: MarkCohortFullCommand) -> None:
