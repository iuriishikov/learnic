from dataclasses import dataclass
from typing import Final, final

from learnic.application.common.auth.authorizer import Authorizer, AuthzTarget
from learnic.application.common.errors import (
    EntityNotFoundError,
    ProductNotArchivedError,
)
from learnic.application.common.persistence.product import ProductGateway
from learnic.application.common.persistence.transaction import Transaction
from learnic.application.common.product_events import (
    ProductEventBus,
    ProductEventKind,
    publish_product_event,
)
from learnic.entities.product.enums import ProductStatus
from learnic.entities.product.ids import ProductID
from learnic.entities.role.permissions import Permission
from learnic.entities.user.models import UserID


@dataclass(slots=True, frozen=True)
class UnarchiveProductCommand:
    actor_id: UserID
    product_id: ProductID


@final
class UnarchiveProductCommandHandler:
    """Restore an archived product to its prior lifecycle state.

    Allowed only when the product is currently archived. The target
    status is derived from ``published_at`` — see
    :meth:`Product.unarchive`. For non-archived products this raises
    :class:`ProductNotArchivedError`.
    """

    def __init__(
        self,
        transaction: Transaction,
        authorizer: Authorizer,
        product_gateway: ProductGateway,
        event_bus: ProductEventBus,
    ) -> None:
        self._transaction: Final = transaction
        self._authorizer: Final = authorizer
        self._product_gateway: Final = product_gateway
        self._event_bus: Final = event_bus

    async def run(self, data: UnarchiveProductCommand) -> None:
        product = await self._product_gateway.with_id(data.product_id)
        if product is None:
            raise EntityNotFoundError(data.product_id)
        await self._authorizer.require(
            data.actor_id,
            AuthzTarget.for_product(data.product_id),
            Permission.ARCHIVE,
        )
        if product.status is not ProductStatus.ARCHIVED:
            raise ProductNotArchivedError(
                data.product_id,
                product.status.value,
            )
        product.unarchive()
        await self._transaction.commit()
        await publish_product_event(
            self._event_bus,
            kind=ProductEventKind.UNARCHIVED,
            product_id=product.oid,
            actor_id=data.actor_id,
            payload={"status": product.status.value},
        )
