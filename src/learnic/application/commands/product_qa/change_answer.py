...
from learnic.entities.product.value_objects import QAAnswer
from learnic.entities.role.permissions import Permission
from learnic.entities.user.models import UserID


@dataclass(slots=True, frozen=True)
class ChangeProductQAAnswerCommand:
    actor_id: UserID
    qa_id: ProductQAID
    value: str
...
@final
class ChangeProductQAAnswerCommandHandler:
    def __init__(
        self,
    ...
    async def run(self, data: ChangeProductQAAnswerCommand) -> None:
        ...
        new_answer = QAAnswer(data.value)
        qa.change_answer(new_answer)
        await self._transaction.commit()
        await publish_product_event(
            self._event_bus,
            kind=ProductEventKind.QA_ANSWER_CHANGED,
