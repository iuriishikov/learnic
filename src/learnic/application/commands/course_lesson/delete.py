...
class DeleteCourseLessonCommand:
    actor_id: UserID
    lesson_id: CourseLessonID


@final
class DeleteCourseLessonCommandHandler:
    """Hard-delete a lesson. Cascades to its blocks (Phase 2+)."""
    ...
    async def run(self, data: DeleteCourseLessonCommand) -> None:
        ...
        await publish_content_event(
            self._event_bus,
            kind=ContentEventKind.LESSON_DELETED,
            product_id=product_id,
            actor_id=data.actor_id,
            payload={"lesson_id": str(data.lesson_id)},
        )
