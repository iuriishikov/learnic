...
class UpdateFileBlockCommand:
    ...
    * ``upload`` present → stream a new file and attach it; the
      previously-attached file is soft-deleted via
      :class:`FileUploadService.soft_delete_previous`. The upload's
      ``size`` and ``content_type`` come from ``upload``; its bytes
      are streamed straight to storage.
    * Absent → only ``title`` is touched (set / clear).
