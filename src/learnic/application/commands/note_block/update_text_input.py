...
class UpdateTextInputBlockCommand:
    """Replace the accepted-answer list and normalisation flags."""
...
@final
class UpdateTextInputBlockCommandHandler:
    def __init__(
        self,
    ...
    async def run(self, data: UpdateTextInputBlockCommand) -> None:
        block = await load_typed_block_for_edit(
            block_id=data.block_id,
            actor_id=data.actor_id,
            expected_type=BlockType.TEXT_INPUT,
            expected_cls=TextInputBlock,
        ...
        block.replace_answers(
            [AcceptedAnswer(a) for a in data.accepted_answers],
            case_sensitive=data.case_sensitive,
            trim_whitespace=data.trim_whitespace,
        )
        await self._block_gateway.update_text_input(block)
