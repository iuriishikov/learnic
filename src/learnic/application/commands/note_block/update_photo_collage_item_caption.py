...
class UpdatePhotoCollageItemCaptionCommand:
    """Edit one photo's caption (or clear it with ``caption=None``)."""
...
@final
class UpdatePhotoCollageItemCaptionCommandHandler:
    def __init__(
        self,
    ...
    async def run(self, data: UpdatePhotoCollageItemCaptionCommand) -> None:
        ...
        try:
            block.update_item_caption(data.item_id, caption)
        except CollageItemsMismatchError as exc:
            raise EntityNotFoundError(data.item_id) from exc
        await self._block_gateway.update_photo_collage_item_caption(
            block,
            data.item_id,
        )
