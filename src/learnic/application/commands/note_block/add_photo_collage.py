...
class CollageItemUpload:
    """One photo upload — a streamed upload + optional caption.

    ``upload`` exposes the streamed bytes plus their ``size`` and
    ``content_type``. The HTTP layer pairs each multipart ``file``
    field with the corresponding positional ``caption`` field; order
    in the ``items`` tuple becomes the persisted collage order.
    """
...
class AddPhotoCollageBlockCommandHandler:
    ...
    async def _validate_and_upload(
        ...
        for src in items:
            if not src.upload.content_type.startswith(
                _IMAGE_CONTENT_TYPE_PREFIX,
            ):
                raise WrongFileContentTypeError(
                    file_id="<upload>",
                    expected_prefix=_IMAGE_CONTENT_TYPE_PREFIX,
                    actual=src.upload.content_type,
                )
        total_bytes = sum(src.upload.size for src in items)
