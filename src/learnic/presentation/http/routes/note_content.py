...
class AddNoteModuleSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/modules``."""
...
class RenameNoteModuleSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/modules/{module_id}/title``."""
...
class UpdateNoteModuleDescriptionSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/modules/{module_id}/description``.
...
class ReorderNoteModulesSchema(BaseModel):
    """Body for ``PUT /notes/{note_id}/modules/order``."""
    ...
    ordered_ids: list[UUID] = Field(
        description=(
            ...
            "match the existing module set of the note exactly."
        ),
        examples=[
            [
                "a1b2c3d4-5e6f-4a8b-9c0d-1e2f3a4b5c6d",
...
class CreatedNoteModuleSchema(BaseModel):
    """Response for ``POST /notes/{note_id}/modules``."""
...
class AddNoteLessonSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/modules/{module_id}/lessons``."""
...
class RenameNoteLessonSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/lessons/{lesson_id}/title``."""
...
class MoveNoteLessonSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/lessons/{lesson_id}/move``.

    Moves the lesson to the end of the target module within the
    same note. Cross-note moves are rejected with HTTP 409.
    """
...
class ReorderNoteLessonsSchema(BaseModel):
    """Body for ``PUT /notes/{note_id}/modules/{module_id}/lessons/order``."""
...
class CreatedNoteLessonSchema(BaseModel):
    """Response for ``POST /notes/{note_id}/modules/{module_id}/lessons``."""
...
class NoteDraftLessonSchema(BaseModel):
...
class NoteDraftModuleSchema(BaseModel):
    ...
    position: int = Field(examples=[0])
    lessons: list[NoteDraftLessonSchema]
    ...
    def from_view(cls, view: DraftModuleView) -> Self:
        return cls(
            ...
            position=view.position,
            lessons=[NoteDraftLessonSchema.from_view(ls) for ls in view.lessons],
        )


class NoteDraftSchema(BaseModel):
    """Full draft tree response for ``GET /notes/{note_id}/content/draft``."""
    ...
    note_id: UUID
    modules: list[NoteDraftModuleSchema]
    fetched_at: datetime

    @classmethod
    def from_view(cls, view: NoteDraftView, fetched_at: datetime) -> Self:
        return cls(
            note_id=view.product_id,
            modules=[NoteDraftModuleSchema.from_view(m) for m in view.modules],
...
@router.post(
    "/{note_id}/modules",
    summary="Add a module to a note product",
    operation_id="addNoteModule",
    status_code=status.HTTP_201_CREATED,
    dependencies=_AUTH_SECURITY,
    response_model=CreatedNoteModuleSchema,
    error_map=_NOTE_AUTHOR_MAP,
)
async def add_module(
    request: Request,
    payload: AddNoteModuleSchema,
    interactor: FromDishka[AddNoteModuleCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
) -> CreatedNoteModuleSchema:
    """Append a module to the note's draft. Author-only.

    Args:
        request: Source of the access cookie.
        payload: Module fields validated by ``AddNoteModuleSchema``.
        interactor: Injected add-module handler.
        auth: Injected authenticator.
        note_id: Note product's UUID.
    ...
    return CreatedNoteModuleSchema(oid=oid)


@router.patch(
    "/{note_id}/modules/{module_id}/title",
    summary="Rename a module",
    operation_id="renameNoteModule",
...
async def rename_module(
    request: Request,
    payload: RenameNoteModuleSchema,
    interactor: FromDishka[RenameNoteModuleCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    ...
    Args:
        ...
        payload: ``{"title": "..."}`` validated by ``RenameNoteModuleSchema``.
        interactor: Injected rename handler.
        auth: Injected authenticator.
        note_id: Note product's UUID (for URL clarity; ownership
    ...
    await interactor.run(
        RenameNoteModuleCommand(
            actor_id=ctx.user_id,
            module_id=NoteModuleID(module_id),
            title=payload.title,
        ),
    )
...
@router.patch(
    "/{note_id}/modules/{module_id}/description",
    summary="Update or clear a module's description",
    operation_id="updateNoteModuleDescription",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP,
)
async def update_module_description(
    request: Request,
    payload: UpdateNoteModuleDescriptionSchema,
    interactor: FromDishka[UpdateNoteModuleDescriptionCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    module_id: Annotated[UUID, _MODULE_ID_PATH],
) -> None:
...
@router.put(
    "/{note_id}/modules/order",
    summary="Replace module ordering atomically",
    operation_id="reorderNoteModules",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP
...
async def reorder_modules(
    request: Request,
    payload: ReorderNoteModulesSchema,
    interactor: FromDishka[ReorderNoteModulesCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
) -> None:
    ...
    Args:
        request: Access cookie source.
        payload: ``{"ordered_ids": [...]}`` — full module list of the note.
        interactor: Injected handler.
        auth: Injected authenticator.
        note_id: Note product's UUID.
    ...
    await interactor.run(
        ReorderNoteModulesCommand(
            actor_id=ctx.user_id,
            product_id=ProductID(note_id),
            ordered_ids=[NoteModuleID(oid) for oid in payload.ordered_ids],
        ),
    )


@router.delete(
    "/{note_id}/modules/{module_id}",
    summary="Delete a module (cascades to its lessons)",
    operation_id="deleteNoteModule",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP,
)
async def delete_module(
    request: Request,
    interactor: FromDishka[DeleteNoteModuleCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    module_id: Annotated[UUID, _MODULE_ID_PATH],
) -> None:
...
@router.post(
    "/{note_id}/modules/{module_id}/lessons",
    summary="Add a lesson to a module",
    operation_id="addNoteLesson",
    status_code=status.HTTP_201_CREATED,
    dependencies=_AUTH_SECURITY,
    response_model=CreatedNoteLessonSchema,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP,
)
async def add_lesson(
    request: Request,
    payload: AddNoteLessonSchema,
    interactor: FromDishka[AddNoteLessonCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    module_id: Annotated[UUID, _MODULE_ID_PATH],
) -> CreatedNoteLessonSchema:
    """Append a lesson to the module. Author-only.

    Args:
        request: Access cookie source.
        payload: Lesson fields validated by ``AddNoteLessonSchema``.
        interactor: Injected handler.
        auth: Injected authenticator.
        note_id: Note product's UUID.
        module_id: Target module UUID.
    ...
    Raises:
        ...
        NotResourceOwnerError: HTTP 403.
        EntityNotFoundError: HTTP 404.
        ResourceLimitReachedError: HTTP 409 — the module already has
            ``NOTE_LESSON_LIMIT`` lessons.
        FieldError: HTTP 422.
    """
    del note_id
    ctx = await auth.authenticate(request)
    ...
    return CreatedNoteLessonSchema(oid=oid)


@router.patch(
    "/{note_id}/lessons/{lesson_id}/title",
    summary="Rename a lesson",
    operation_id="renameNoteLesson",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP,
)
async def rename_lesson(
    request: Request,
    payload: RenameNoteLessonSchema,
    interactor: FromDishka[RenameNoteLessonCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
...
@router.patch(
    "/{note_id}/lessons/{lesson_id}/move",
    summary="Move a lesson to another module within the same note",
    operation_id="moveNoteLesson",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP
    | {CrossNoteLessonMoveError: CROSS_NOTE_LESSON_MOVE_RULE},
)
async def move_lesson(
    request: Request,
    payload: MoveNoteLessonSchema,
    interactor: FromDishka[MoveNoteLessonCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    lesson_id: Annotated[UUID, _LESSON_ID_PATH],
) -> None:
    """Move the lesson to ``target_module_id`` (must belong to the same note).
    ...
    Raises:
        ...
        CrossNoteLessonMoveError: HTTP 409 — target module belongs
            to a different note.
    """
    del note_id
    ctx = await auth.authenticate(request)
    await interactor.run(
        MoveNoteLessonCommand(
            actor_id=ctx.user_id,
            lesson_id=NoteLessonID(lesson_id),
            target_module_id=NoteModuleID(payload.target_module_id),
        ),
    )


@router.put(
    "/{note_id}/modules/{module_id}/lessons/order",
    summary="Replace lesson ordering inside a module atomically",
    operation_id="reorderNoteLessons",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP
    | {InvalidReorderError: INVALID_REORDER_RULE},
...
async def reorder_lessons(
    request: Request,
    payload: ReorderNoteLessonsSchema,
    interactor: FromDishka[ReorderNoteLessonsCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    module_id: Annotated[UUID, _MODULE_ID_PATH],
) -> None:
    ...
    await interactor.run(
        ReorderNoteLessonsCommand(
            actor_id=ctx.user_id,
            module_id=NoteModuleID(module_id),
            ordered_ids=[NoteLessonID(oid) for oid in payload.ordered_ids],
        ),
    )


@router.delete(
    "/{note_id}/lessons/{lesson_id}",
    summary="Delete a lesson",
    operation_id="deleteNoteLesson",
    status_code=status.HTTP_204_NO_CONTENT,
    dependencies=_AUTH_SECURITY,
    error_map=AUTHENTICATED_OWNER_FIELD_MAP,
)
async def delete_lesson(
    request: Request,
    interactor: FromDishka[DeleteNoteLessonCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
    lesson_id: Annotated[UUID, _LESSON_ID_PATH],
) -> None:
...
@router.get(
    "/{note_id}/content/draft",
    summary="Read the full draft tree of a note",
    operation_id="getNoteDraft",
    response_model=NoteDraftSchema,
    dependencies=_AUTH_SECURITY,
    error_map=_NOTE_AUTHOR_MAP,
)
async def get_draft(
    request: Request,
    interactor: FromDishka[GetNoteDraftQueryHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
) -> NoteDraftSchema:
    """Return the full draft tree of the note (modules + lessons + blocks).
    ...
    Returns:
        :class:`NoteDraftSchema` with modules ordered by position,
    ...
    return NoteDraftSchema.from_view(
        view,
...
class AddHtmlBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/html``.
...
class UpdateHtmlBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/html``."""
...
class AddKatexBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/katex``."""
...
class UpdateKatexBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/katex``."""
...
class AddCodeBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/code``.
...
class UpdateCodeBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/code``.
...
class AddSingleChoiceBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/single-choice``.
...
class UpdateSingleChoiceBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/single-choice``.
...
class AddMultiChoiceBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/multi-choice``.
...
class UpdateMultiChoiceBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/multi-choice``."""
...
class AddTextInputBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/text-input``.
...
class UpdateTextInputBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/text-input``."""
...
class ReorderLessonBlocksSchema(BaseModel):
    """Body for ``PUT /notes/{note_id}/lessons/{lesson_id}/blocks/order``."""
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/html",
...
async def add_html_block(
    ...
    Args:
        ...
        interactor: Injected handler.
        auth: Injected authenticator.
        note_id: Note product UUID (for URL clarity).
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/katex",
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/code",
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/single-choice",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/single-choice",
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/multi-choice",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/multi-choice",
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/text-input",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/text-input",
...
@router.put(
    "/{note_id}/lessons/{lesson_id}/blocks/order",
...
class AddRutubeVideoBlockSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/lessons/{lesson_id}/blocks/rutube-video``.
...
class UpdateRutubeVideoBlockSchema(BaseModel):
    """Body for ``PATCH /notes/{note_id}/blocks/{block_id}/rutube-video``.
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/rutube-video",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/rutube-video",
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/file",
...
async def add_file_block(
    ...
    Args:
        ...
        auth: Injected authenticator.
        note_id: Note product UUID (framing only — handler authorises
...
@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/video-file",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/video-file",
...
async def _read_collage_items(
    ...
    for f, c in pairs:
        ...
        out.append(CollageItemUpload(upload=upload, caption=c))
    return tuple(out)


@router.post(
    "/{note_id}/lessons/{lesson_id}/blocks/photo-collage",
...
class ReorderPhotoCollageItemsSchema(BaseModel):
    """Body for ``PUT /notes/{note_id}/blocks/{block_id}/photo-collage/items/order``.
...
@router.post(
    "/{note_id}/blocks/{block_id}/photo-collage/items",
...
async def add_photo_collage_item(
    ...
    Args:
        request: Source of the access cookie.
        note_id: URL framing — target note UUID.
...
@router.delete(
    "/{note_id}/blocks/{block_id}/photo-collage/items/{item_id}",
...
async def remove_photo_collage_item(
    ...
    del note_id
    ctx = await auth.authenticate(request)
    from learnic.entities.note_block.ids import (  # noqa: PLC0415
        CollageItemID,
    )
...
@router.put(
    "/{note_id}/blocks/{block_id}/photo-collage/items/order",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/photo-collage/items/{item_id}/caption",
...
@router.patch(
    "/{note_id}/blocks/{block_id}/photo-collage/title",
...
class ResetNoteDraftSchema(BaseModel):
    """Body for ``POST /notes/{note_id}/draft/reset``.

    Identifies the release whose snapshot the draft should be
    rehydrated from. The release must belong to the same note;
    otherwise HTTP 404.
    """
    ...
    release_id: UUID = Field(
        description=(
            ...
            "current draft. Must belong to the same note."
        ),
...
async def reset_draft(
    request: Request,
    payload: ResetNoteDraftSchema,
    interactor: FromDishka[ResetNoteDraftCommandHandler],
    auth: FromDishka[Authenticator],
    note_id: Annotated[UUID, _NOTE_ID_PATH],
) -> None:
