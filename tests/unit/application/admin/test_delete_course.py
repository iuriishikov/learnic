...
def _handler(
    ...
    return AdminDeleteCourseCommandHandler(
...
async def test_delete_course_cascades_files_commits_and_publishes(
    fake_transaction: AsyncMock,
    fake_product_gateway: AsyncMock,
    fake_files_reader: AsyncMock,
    fake_file_uploads: AsyncMock,
    ...
    await handler.run(
        AdminDeleteCourseCommand(
            ...
            course_id=ProductID(fake_product.oid),
        ),
    )
...
async def test_delete_course_unknown_raises_and_does_not_commit(
    fake_transaction: AsyncMock,
    fake_product_gateway: AsyncMock,
    fake_files_reader: AsyncMock,
    fake_file_uploads: AsyncMock,
    ...
    with pytest.raises(EntityNotFoundError):
        await handler.run(
            AdminDeleteCourseCommand(
                actor_id=UserID(uuid.uuid4()),
                course_id=ProductID(uuid.uuid4()),
            ),
        )
