...
class TestNoteReleaseVersion:
    ...
    def test_rejects_negative_components(self) -> None:
        with pytest.raises(NegativeReleaseVersionError):
            NoteReleaseVersion(-1, 0, 0)
        with pytest.raises(NegativeReleaseVersionError):
            NoteReleaseVersion(0, -1, 0)
        with pytest.raises(NegativeReleaseVersionError):
            NoteReleaseVersion(0, 0, -1)
